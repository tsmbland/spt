...
def_roi should have slider - steal from MembraneQuant code
GUI
DiffusionFitter and OffRateFitter needn't be extensions of ParticleFinder


Notes for GUI:
Button to import
Optional button to specify ROI
Parameters: Particle identification (featsize, threshold), Tracking (memory, mintracklength, dist), Computation (parallel), Start frame, end frame
Widget for choosing threshold and featsize
Button to perform tracking
Widget to view tracking results
Button to save results (should it save feats too?)
...
class SPT:
    ...
    def __init__(self, direc=None, roi=None, threshold=600, feat_size=5, dist=5, memory=3, mtl=50, start_frame=0,
                 end_frame=None, parallel=False, cores=None, save_path=None):
    ...
    def find_feats(self):
        ...
        if self.parallel:
            ...
            self.features_all = tp.batch(self.frames[:], self.feat_size,
...
class DiffusionFitter:
    ...
    def __init__(self, trajectories, timestep, pixel_size, n_frames):
    ...
    def analyze_tracks(self):
        ...
        self.im = tp.imsd(self.trajectories, mpp=self.pixelSize, fps=1 / self.timestep, max_lagtime=100)
